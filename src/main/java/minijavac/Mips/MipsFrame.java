...
import minijavac.Assem.Codegen;
import minijavac.Assem.Instr;
import minijavac.Assem.InstrList;
import minijavac.Assem.OPER;
...
public class MipsFrame extends Frame {
    ...
    public List<Instr> codegen(StmList stms) {
        List<Instr> instrs = new LinkedList<>();
        Codegen cg = new Codegen(this);
        StmList currStms = stms;

        while(currStms != null) {
            InstrList instrList = cg.codegen(currStms.head);
            while(instrList != null) {
                instrs.add(instrList.head);
                instrList = instrList.tail;
            }
            currStms = currStms.tail;
        }
    ...
    private static Instr OPER(String a, Temp[] d, Temp[] s) {
        return new OPER(a, new TempList(d), new TempList(s), null);
    }
    ...
    public void spill(List<Instr> insns, Temp[] spills) {
        if (spills != null) {
            if (!spilling) {
                ...
                Expr exp = allocLocal(true).exp(TEMP(FP));
                    ...
                    if (use != null)
                        for (int u = 0; u < use.length; u++) {
                            if (use[u] == spills[s]) {
                                ...
                                i.previous();
//                                stm.accept(new Codegen(this, i)); visitor pro Stm ??????
                                if (insn != i.next())
                                    throw new Error();
